...
/// 对Function进行扩展
extension YuliaFunctionExt on Function {
  VoidCallback throttle() {
    return YuliaFunctionProxy(this).throttle;
  }

  VoidCallback throttleWithTimeout({int? timeout}) {
    return YuliaFunctionProxy(this, timeout: timeout).throttleWithTimeout;
  }

  VoidCallback debounce({int? timeout}) {
    return YuliaFunctionProxy(this, timeout: timeout).debounce;
  }
}

/// 节流、防抖 实现方式
class YuliaFunctionProxy {
  ...
  YuliaFunctionProxy(this.target, {int? timeout}) : timeout = timeout ?? 500;
